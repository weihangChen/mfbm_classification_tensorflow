...
class FBM(object):
    ...
    def __init__(self, n, hurst,  length=1, method="daviesharte", hurst_actions = None):
        """Instantiate the FBM."""
    ...
    def adjust_hurst(self, k):
        #if (k==1):
        #    print("----------------------------DEBUG START----------------------------")
        #    print(self.hurst)
        if 150 < k <= 200 and (self.hurst_actions[0] is not None): 
            adjusted_hurst = self.hurst_actions[0].get_hurst(k)
            self.hurst = adjusted_hurst
            #if (k==200):
            #    print(self.hurst)
        elif 350 < k <= 400 and (self.hurst_actions[1] is not None): 
            adjusted_hurst = self.hurst_actions[1].get_hurst(k)
            self.hurst = adjusted_hurst
            #if (k==400):
            #    print(self.hurst)
            #    print("----------------------------DEBUG END----------------------------")
...
def fbm(n, hurst, length=1, method="daviesharte", hurst_actions = None):
    """One off sample of fbm."""
    f = FBM(n, hurst, length, method,hurst_actions)
    return f.fbm()
...
#n1 = 600
#daviesharte, hosking, cholesky
#actions senario1, up from 0.6 to 0.65 at 150, down from 0.65 to 0.6 at 350, 0.6 => 0.65 => 0.6
#start = 0.6
#action1 = HurstUpOrDown(start, 150, 600, "up")
#action2 = HurstUpOrDown(0.6499999999999999, 350, 600, "down")
#hurst_actions = [action1, action2]
#actions senario2, down from 0.6499999999999999 to 0.6 at 150, up from 0.6 to 0.65 at 350, 0.65=>0.6=>0.65

#start = 0.6499999999999999
#action1 = HurstUpOrDown(start, 150, 600, "down")
#action2 = HurstUpOrDown(0.6, 350, 600, "up")
#hurst_actions = [action1,action2]


#for x in range(1,2):
#    serie = fbm(n1, start, 1, 'daviesharte', hurst_actions)
#    #print(serie)
#    plt.plot(list(range(0, n1+1)), serie)
